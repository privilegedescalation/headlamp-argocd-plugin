...
- This token grants `get` on `services/proxy` (resourceName `https:argocd-server:443`) and `get` on `services` (resourceName `argocd-server`), both in the `argocd` namespace only. The Kubernetes RBAC resourceName for a `services/proxy` subresource is the scheme-qualified form `<scheme>:<service>:<port>`; for the default config that is `https:argocd-server:443`, not bare `argocd-server`. Both resourceName forms are required (PRI-1956 dual-resourceName fix). See the `headlamp-uat-reader` Role/RoleBinding for the full grant (PRI-1951).
